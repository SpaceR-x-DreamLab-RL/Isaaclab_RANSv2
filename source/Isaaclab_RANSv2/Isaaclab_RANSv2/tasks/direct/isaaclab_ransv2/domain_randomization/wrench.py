...
from ..domain_randomization import RandomizationCore, RandomizationCoreCfg, Registerable
from ..utils import PerEnvSeededRNG
...
class WrenchRandomization(Registerable, RandomizationCore):
    ...
    def apply_randomization(self, ids: torch.Tensor | None = None) -> None:
        ...
        projected_forces = quat_apply_inverse(self._asset.data.root_com_quat_w[ids], forces).unsqueeze(1)
        torques = self._constant_torque[ids] + self._kick_torque[ids]
        if self._cfg.torque_dimension != 1:
            projected_torques = quat_apply_inverse(self._asset.data.root_com_quat_w[ids], torques).unsqueeze(1)
        else:
